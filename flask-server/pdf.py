...
def filetoBytesIO(filepath):
    with open(filepath, "rb") as fh:
        buf = BytesIO(fh.read())
        return buf

def pdftotext(url, pdftype="url"):
    if pdftype == "url":
        pdf_file_on_mem = grabfiletomemory(url)
    elif pdftype == "file":
        pdf_file_on_mem = filetoBytesIO(url)
    ocr = ''
    if (pdf_file_on_mem):
...
#main("/Users/mahitnamburu/Desktop/PDFJudicial/flask-server/uploads/template.csv")


#create a dataframe with url,ocr,description,summary,type,subject,locations,agencies,people
def main2(file, types):
    # Gather all data necessary for csv file
    pdfs = pd.DataFrame()
    pdfs['url'] = [file]
    
    ocr = pdftotext(file, types)
    pdfs['ocr'] = [ocr]
    
    description = llama("Write a brief summary in three sentences or less about these data.", ocr)
    description = description.split("\n")
    description = description[-1]    
    pdfs['description'] = [description]    
    print("Description Finished")
    
    summary = llama("Summarize these data", ocr)
    summary = summary.split("\n")
    summary = summary[1:]
    pdfs['summary'] = [summary]
    print("Summary Finished")

    types = llama("In one word, classify what type of document this is", ocr)
    pdfs['type'] = [types]
    print("Type Finished")

    subject = llama("Extract subject matter tags related to law from these data. Separate the list by comma", ocr)
    pdfs['subject'] = [subject]
    print("Subject Finished")

    locations = llama("Extract locations from these data.  Separate by comma", ocr)
    pdfs['locations'] = [locations]
    print("Locations Finished")

    agencies = llama("Extract agencies and department from these data.  Separate by comma", ocr)
    pdfs['agencies'] = [agencies]
    print("Agencies Finished")

    people = llama("Extract names of people from these data.  Separate by comma", ocr)
    pdfs['people'] = [people]
    print("People Finished")
    
    #return a file and save it
    pdfs.to_csv('./downloads/info.csv', index=False, quotechar='"')
    pdfarr = [file, ocr, description, summary, types, subject, locations, agencies, people]
    print(pdfarr)
    
    # convert list of elements into a CSV format for easier parsing
    return_string = ''
    for elem in pdfarr:
        # checking for inner lists likely needs to be more comprehensive
        if (type(elem) == list):
            elem = ' '.join([str(element) for element in elem])
